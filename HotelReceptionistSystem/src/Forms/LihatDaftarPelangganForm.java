...
public class LihatDaftarPelangganForm {
	...
	private void Initialize() {
		...
		frmLihatDaftarPelangganForm.setBounds(100, 100, 800, 400);
	...
	private void SetupTableDaftarPelanggan() {
		String[] columnNames = new String[] { "No", "Pelanggan", "Nomor Kamar", "Tanggal Cek In", "Lama Menginap", "Email", "Nomor Telepon" };
		...
		scrollTableDaftarPelanggan.setBounds(12, 12, 760, 262);
	...
	private void LoadDataTableDaftarPelanggan() {
		...
		for (Transaction transaction : transactionsData) {
			tableModel.addRow(new Object[] { number, transaction.getCustomer().getName(), transaction.getRoomNumber(), DateHelper.formatDate(transaction.getCheckInDate(), "dd-MM-yyyy"), transaction.getLengthOfStay() + " hari", transaction.getCustomer().getEmail(), transaction.getCustomer().getPhoneNumber() });
			number++;
		}
